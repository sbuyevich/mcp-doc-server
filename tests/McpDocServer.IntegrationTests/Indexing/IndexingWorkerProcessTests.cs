...
public sealed class IndexingWorkerProcessTests
{
    [Fact]
    public async Task OneShotWorkerIndexesLocalPackageAndExitsZero()
    {
        var root = CreateRoot();
        ...
        try
        {
            var result = await RunWorkerAsync(feed, databasePath);
    ...
    [Fact]
    public async Task OneShotWorkerReturnsOneWhenPackageFails()
    {
        var root = CreateRoot();
        var feed = Path.Combine(root, "feed");
        var databasePath = Path.Combine(root, "index", "docs.db");
    ...
    [Fact]
    public async Task OneShotWorkerReturnsOneWhenIndexingPartiallySucceeds()
    {
    ...
    private static string CreateRoot() =>
        Path.Combine(
            Path.GetTempPath(),
            $"mcp-doc-server-worker-tests-{Guid.NewGuid():N}");
    ...
    private static string WorkerAssemblyPath()
    {
        ...
        return Path.Combine(
            ...
            buildConfiguration,
            "net10.0",
            "McpDocServer.Indexing.Worker.dll");
    }
    ...
    private sealed record WorkerResult(int ExitCode, string Logs);
}
